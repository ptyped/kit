...
module.exports = (config, app) => {
    ...
    staticRoutes.forEach(r => addStaticRoutes(r, config.dirs.views, config, app))
    dynamicRoutes.forEach(r => addDynamicRoutes(r, config.dirs.views, config, app))
}
        ...
        routes.forEach(r => {
            ...
            if (route && template) {
                router.use(route, (req, res, next) => {
                    renderPathWithFrontMatter(config, template, res, next)
                })
            }
        })
    }
