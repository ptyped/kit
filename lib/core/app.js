...
class App {
  ...
  configureStaticFiles() {
    ...
    this.use(publicPath, express.static(dependencyDir), serveIndex(dependencyDir, {'icons': true}))
    this.use(publicPath, express.static(staticDir), serveIndex(staticDir, {'icons': true}))

    debug('Serving static files from `%s`, `%s` at: %s', dependencyDir, staticDir, publicPath)
  }
