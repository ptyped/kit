...
const addGlobalDataFiles = (config, req, res) => {
    const globPattern = path.join(config.dirs.data, `**/*.{${acceptedExts.join(",")}}`)
...
const addLocalDataFiles = (config, req, res) => {
    const globPattern = path.join(config.dirs.views, `**/data.{${acceptedExts.join(",")}}`)
    const dataFiles = glob.sync(globPattern)

    dataFiles.forEach(file => {
        const fileRelPath = file.replace(config.dirs.views, "")
