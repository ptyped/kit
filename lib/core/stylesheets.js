...
const debug = require('debug')('pkit:stylesheets');
...
module.exports.sass = async (config, watch) => {
  ...
  if (watch) {
    chokidar
      .watch(globPattern, { ignoreInitial: true })
      .on('change', path => processSassEntryPoint(config, path));
  }
...
module.exports.postcss = async (config, watch) => {
  ...
  if (watch) {
    chokidar
      .watch(globPattern, { ignoreInitial: true })
      .on('change', path => processPostCssEntryPoint(config, path, false));
  }
...
const processSassEntryPoint = async (config, entryPoint) => {
  ...
  debug('Sass `%s` to: %s', entryPoint, outPath);
...
const processPostCssEntryPoint = async (config, entryPoint, isAfterSass) => {
  ...
  const postcssConfig = merge(defaultConfig, config.get('configurePostcss'));
  ...
  debug('Postcss `%s` to: %s', entryPoint, outPath);
