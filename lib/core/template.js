...
class Template {
    ...
    createProject() {
        return new Promise((resolve, reject) => {
            ...
            if (!fs.existsSync(projectPkgPath)) {
                ...
                debug("Using default `package.json`: %o", projectPkg)
            } else {
                projectPkg = JSON.parse(fs.readFileSync(projectPkgPath))
                debug("Using existing `package.json`: %o", projectPkg)
            }
    ...
    copy() {
        return new Promise((resolve, reject) => {
            ...
            const templateFiles = glob.sync(templateSrcPath + "/**/*").filter(file => fs.lstatSync(file).isDirectory());
    ...
    async install() {
        try {
            debug("Installing project dependencies: %s, %s", this.config.get('kitModule'), this.dependency.input)
