 ...
 */
module.exports.create = async (config, dependencyPath, dependencyPrefix) => {
    const dependencyType = dependencyTypes[resolveDependencyType(dependencyPath)]
    const dependenciesPath = path.resolve(__dirname, dependencyPrefix ? dependencyPrefix : "", "../../dependencies")
    ...
    if (dependencyType === dependencyTypes[3]) {
        dependencyPath = path.resolve(process.cwd(), dependencyPath.substr(5))
    }
