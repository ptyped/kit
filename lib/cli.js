...
(async () => {
  ...
  const { build, init, updateDependencies } = require('./index');
  let { getFromProject, getConfigArgs } = require('./core/utils');
  ...
  Object.keys(args).forEach(key => {
    program.option(`--${args[key].arg}`, args[key].doc);
  });
  ...
  program.command('start').action(() => {
    ...
    const nodemonOpts = {
      script: serverPath,
      ext: 'js json yml',
      watch: [__dirname, viewsPath, dataPath],
      env: {
    ...
    server.on('quit', () => debug('Nodemon stopped'));
    server.on('restart', files =>
      debug('Nodemon resarted due to changes in files: %o', files)
    );
  });
   ...
   * Build command
   */
  program.command('build').action(() => build(localConfig));
