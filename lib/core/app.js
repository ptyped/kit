...
const middlewareBasicAuth = require('./middleware/basicAuth');
...
class App {
  ...
  configureViewEngines() {
    const relKitViews = path.resolve(__dirname, '../views');
    const projectKitViews = path.resolve(
      this.getFromProject('node_modules'),
      pkg.name,
      'lib/views'
    );
    let views = [relKitViews, projectKitViews, this.config.get('dirs.views')];
  ...
  configureAuth() {
    const username = this.config.get('auth.username')
    const password = this.config.get('auth.password')

    if (username && password) {
      this.use(middlewareBasicAuth(username, password))
    }
  }
