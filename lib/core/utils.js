 ...
 */
module.exports.copyFromTemplate = async (config, templateDir, projectDir) =>
  new Promise(async (resolve, reject) => {
    const templatePath = templateDir ? templateDir : path.resolve(__dirname, "../template")
    const templateFiles = glob.sync(templatePath + "/**/*");
    const projectPath = projectDir ? projectDir : module.exports.getFromApp(".");

    if (fs.existsSync(templatePath)) {
      if (!fs.existsSync(projectDir)) {
        await ncp(templatePath, projectPath)
      } else {
...
module.exports.renderStaticPath = (config, reqPath, res, next) => {
  ...
  res.render(reqPath, (error, html) => {
    ...
    if (!reqPath.endsWith("/index")) {
      ...
      return module.exports.renderStaticPath(config, reqPath + "/index", res, next);
    }
...
module.exports.getFrontmatterData = (config, reqPath) => {
  ...
  const realPath = path.join(config.dirs.views, reqPath)
