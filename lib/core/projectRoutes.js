...
const { getFromProject, renderStaticPath } = require('./utils');
...
class ProjectRoutes {
  constructor(config) {
    ...
    this.staticRouteFiles = this.routeFiles.filter(
      p => path.extname(p) !== '.js'
    );
    this.dynamicRouteFiles = this.routeFiles.filter(
      p => path.extname(p) === '.js'
    );
  }
  ...
  addStaticRoutes(app) {
    this.staticRouteFiles.forEach(routesFile => {
      ...
      const routes = matter(`---\n${routesContent}\n---`).data;
      const routesPath = this.getRoutesPath(
        routesFile,
      ...
      if (Array.isArray(routes)) {
        routes.forEach(r => {
          let { route, redirect, template, data } = r;
          data = data ? data : {};
