...
const nunjucksView = require("./nunjucksExpressView")
...
module.exports.app = (config, routes) => {
  ...
  nunjucksView(config, nunjucksEnv, app);
   ...
   */
  app.use('/public', express.static(path.resolve(__dirname, "../../dependencies")))
  app.use('/public', express.static(config.get('dirs.static')))
   ...
   */
  app.use(middlewareAutoStoreData(config));

  /**
   * Add global data files to all routes
   */
  app.use(middlewareDataFiles(config))
...
module.exports.start = async (config, routes) => {
  ...
  findAvailablePort(config.get('port'), opts, port => {
    ...
    if (config.get('env') === "production") {
      ...
      app.listen(proxyPort, () => {
        browserSync({
          ...
          ui: false,
          files: [
            config.get('dirs.input'),
            config.get('dirs.static')
          ],
          ghostmode: false,
          open: false,
          notify: false,
