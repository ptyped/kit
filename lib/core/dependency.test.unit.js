...
const paths = [
    ['NPM module', 'ptyped-kit-starter', 0, {
        name: 'ptyped-kit-starter',
        type: "NPM",
        input: 'ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
    }],
    ['Org-scope NPM module', '@ptyped/ptyped-kit-starter', 0, {
        name: '@ptyped/ptyped-kit-starter',
        type: "NPM",
        input: '@ptyped/ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), '@ptyped/ptyped-kit-starter')
    }],
    ['Github short-form', 'ptyped/ptyped-kit-starter', 1, {
        name: 'ptyped-kit-starter',
        type: "GIT",
        input: 'ptyped/ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
      }],
    ['Github prefix', 'github:ptyped/ptyped-kit-starter', 1, {
        name: 'ptyped-kit-starter',
        type: "GIT",
        input: 'github:ptyped/ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
      }],
    ['Bitbucket prefix', 'bitbucket:ptyped/ptyped-kit-starter', 1, {
        name: 'ptyped-kit-starter',
        type: "GIT",
        input: 'bitbucket:ptyped/ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
    }],
    ['Gitlab prefix', 'gitlab:ptyped/ptyped-kit-starter', 1, {
        name: 'ptyped-kit-starter',
        type: "GIT",
        input: 'gitlab:ptyped/ptyped-kit-starter',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
    }],
    ['Direct prefix', 'direct:https://github.com/ptyped/ptyped-kit-starter.git', 2, {
        name: 'ptyped-kit-starter',
        type: "DIRECT",
        input: 'direct:https://github.com/ptyped/ptyped-kit-starter.git',
        output: path.resolve(config.get('dirs.dependencies'), 'ptyped-kit-starter')
    }],
    ['Web [http]', 'http://example.com/filename.png', 3, {
        name: 'filename.png',
        type: "WEB",
        input: 'http://example.com/filename.png',
        output: path.resolve(config.get('dirs.dependencies'), 'filename.png')
    }],
    ['Web [https]', 'https://example.com/filename.png', 3, {
        name: 'filename.png',
        type: "WEB",
        input: 'https://example.com/filename.png',
        output: path.resolve(config.get('dirs.dependencies'), 'filename.png')
    }],
    ['File prefix', 'file:../path/to/file_or_folder', 4, {
        name: 'file_or_folder',
        type: "FILE",
