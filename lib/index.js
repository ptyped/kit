...
module.exports.init = async (userConfig, dir) => {
  try {
    ...
    await configInstance.loadFromProjectPackage()

    config = configInstance.config

    // Clear deps
    const dependencies = config.get('dependencies')
    const globPattern = path.join(configInstance.getFromProject('node_modules'), pkg.name, "../dependencies", "/**/*")
...
module.exports.start = async (userConfig, routes) => {
  try {
    ...
    await configInstance.loadFromTemplateConfig()
    const config = configInstance.config
    const app = module.exports.app(config.getProperties(), routes)
