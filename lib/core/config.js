...
module.exports = (config = {}) => {
    ...
    const template = dependency.create(config, config.get('template'))
    const templateConfig = path.resolve(template.output, ".kit.js")

    if (fs.existsSync(templateConfig)) {
        config.load(require(templateConfig)(config))
    }
