...
class Config {
  constructor(userConfig, cwd) {
    this.schema = {
      ...
      dependencies: {
        doc:
          'Any web-based third-party dependencies to be served as static files',
        format: '*',
        default: [],
        arg: 'dependencies'
      },
      ...
      dirs: {
        ...
        assets: {
          ...
          css: {
            doc: 'The folder containing CSS to be compiled',
            format: '*',
            default: 'assets/css',
            arg: 'cssDir',
            env: "CSS_DIR"
          ...
          scss: {
            doc: 'The folder containing SCSS to be compiled',
            format: '*',
            default: 'assets/css',
            arg: 'scssDir',
            env: "SCSS_DIR"
      ...
      auth: {
        username: {
          doc: "Username to be used for basic authentication",
          format: "String",
          default: undefined,
          arg: "username",
          env: "USERNAME"
        ...
        password: {
          doc: "Password to be used for basic authentication",
          format: "String",
          default: undefined,
          arg: "password",
          env: "PASSWORD"
  ...
  async loadFromTemplateConfig() {
    this.templateDep = await dependency.create(
      this.config,
      this.config.get('template')
    );
    const templateConfigPath = this.getFromProject(
      path.join('node_modules', this.templateDep.name, '.kit.js')
    );
  ...
  resolveDirectoriesFromProject() {
    ...
    const resolveDirs = (dirs, parents) =>
